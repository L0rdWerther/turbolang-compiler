...
class CodeGenerator:
    ...
    def generate(self, program: Program) -> str:
        for func in program.functions:
            self.function_table[func.name] = (func.return_type, [param.name for param in func.parameters])
    ...
    def generate_function(self, func: FunctionDecl) -> None:
        ...
        self.next_local_offset = 2
        self.scopes = [{}]
        self.emit_label(f'FUNCTION_{func.name}')
        offset = -1
    ...
    def generate_do_while_statement(self, do_stmt: DoWhileStatement) -> None:
        start_label = self.new_label('DO_WHILE_START')
        self.emit_label(start_label)
        if do_stmt.body:
            self.push_scope()
    ...
    def generate_return_statement(self, ret_stmt: ReturnStatement) -> None:
        if ret_stmt.value:
            ...
            if self.current_return_type:
                self.emit_assignment_conversion(self.expression_type(ret_stmt.value), self.current_return_type)
    ...
    def generate_binary_expression(self, expr: BinaryExpression) -> None:
        ...
        if use_float:
            instructions = {'+': 'ADDF', '-': 'SUBF', '*': 'TIMESF', '/': 'DIVF', '%': 'MOD'}
        elif left_type == 'float' and right_type == 'float' and (expr.operator in ['==', '!=', '<', '>', '<=', '>=']):
            instructions = {'==': ['CMPF', 'ISNIL'], '!=': ['CMPF', 'ISNIL', 'NOT'], '<': ['CMPF', 'ISNEG'], '>': ['CMPF', 'ISPOS'], '<=': ['CMPF', 'ISPOS', 'NOT'], '>=': ['CMPF', 'ISNEG', 'NOT']}
        else:
            instructions = {'+': 'ADD', '-': 'SUB', '*': 'TIMES', '/': 'DIV', '%': 'MOD', '==': 'EQUAL', '!=': ['EQUAL', 'NOT'], '<': 'LESS', '>': 'GREATER', '<=': ['GREATER', 'NOT'], '>=': ['LESS', 'NOT'], '&&': 'AND', '||': 'OR'}
    ...
    def generate_function_call(self, call: FunctionCall) -> None:
        ...
        self.emit('LINK')
        self.emit(f'JSR FUNCTION_{call.name}')
        self.emit('POPFBR')
        if call.arguments:
    ...
    def expression_type(self, expr: Expression) -> str:
        ...
        if isinstance(expr, BinaryExpression):
            return TypeChecker.binary_operation_type(self.expression_type(expr.left), expr.operator, self.expression_type(expr.right))

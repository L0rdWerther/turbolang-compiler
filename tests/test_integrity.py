...
class TestCompilerIntegrity(unittest.TestCase):
    ...
    def test_project_structure_exists(self):
        required_dirs = ['lexer', 'parser', 'semantic', 'codegen', 'tests', 'examples']
        for dir_name in required_dirs:
            dir_path = self.project_root / dir_name
            self.assertTrue(dir_path.exists() and dir_path.is_dir(), f"Required directory '{dir_name}' not found at {dir_path}")

    def test_lexer_module_files_exist(self):
        required_files = ['lexer/__init__.py', 'lexer/token.py', 'lexer/lexer.py', 'lexer/keywords.py']
        for file_name in required_files:
            file_path = self.project_root / file_name
            self.assertTrue(file_path.exists() and file_path.is_file(), f"Required file '{file_name}' not found at {file_path}")

    def test_parser_module_files_exist(self):
        required_files = ['parser/__init__.py', 'parser/parser.py', 'parser/ast_nodes.py']
        for file_name in required_files:
            file_path = self.project_root / file_name
            self.assertTrue(file_path.exists() and file_path.is_file(), f"Required file '{file_name}' not found at {file_path}")

    def test_semantic_module_files_exist(self):
        required_files = ['semantic/__init__.py', 'semantic/symbol_table.py', 'semantic/type_checker.py', 'semantic/semantic_analyzer.py']
        for file_name in required_files:
            file_path = self.project_root / file_name
            self.assertTrue(file_path.exists() and file_path.is_file(), f"Required file '{file_name}' not found at {file_path}")

    def test_codegen_module_files_exist(self):
        required_files = ['codegen/__init__.py', 'codegen/sam_instructions.py', 'codegen/code_generator.py']
        for file_name in required_files:
            file_path = self.project_root / file_name
            self.assertTrue(file_path.exists() and file_path.is_file(), f"Required file '{file_name}' not found at {file_path}")

    def test_root_level_files_exist(self):
        required_files = ['main.py', 'compiler.py', 'README.md']
        for file_name in required_files:
            file_path = self.project_root / file_name
            self.assertTrue(file_path.exists() and file_path.is_file(), f"Required file '{file_name}' not found at {file_path}")

    def test_example_files_exist(self):
        required_examples = ['examples/hello.turbo', 'examples/fibonacci.turbo', 'examples/factorial.turbo', 'examples/array_sum.turbo', 'examples/control_flow.turbo']
        for file_name in required_examples:
            file_path = self.project_root / file_name
            self.assertTrue(file_path.exists() and file_path.is_file(), f"Required example '{file_name}' not found at {file_path}")

    def test_test_files_exist(self):
        required_tests = ['tests/test_lexer.py', 'tests/test_parser.py', 'tests/test_semantic.py', 'tests/test_codegen.py']
        for file_name in required_tests:
            file_path = self.project_root / file_name
            self.assertTrue(file_path.exists() and file_path.is_file(), f"Required test '{file_name}' not found at {file_path}")
    ...
    def test_parser_imports(self):
        try:
            ...
            from parser.ast_nodes import Program, FunctionDecl, Parameter, Block, Statement, VariableDecl, Assignment, IfStatement, WhileStatement, ReturnStatement, Expression, BinaryExpression, UnaryExpression, FunctionCall, Literal, Identifier
        except ImportError as e:
...
class TestCompilerComponentChecklist(unittest.TestCase):
    ...
    def test_checklist(self):
        ...
        print('=' * 70)
        components = {'Lexer Components': [('lexer/token.py', 'Token and TokenType classes'), ('lexer/lexer.py', 'Lexer with tokenize() method'), ('lexer/keywords.py', 'Keyword definitions')], 'Parser Components': [('parser/parser.py', 'Parser with recursive descent'), ('parser/ast_nodes.py', 'All AST node classes')], 'Semantic Components': [('semantic/symbol_table.py', 'SymbolTable and FunctionTable'), ('semantic/type_checker.py', 'TypeChecker with type rules'), ('semantic/semantic_analyzer.py', 'SemanticAnalyzer')], 'Code Generation': [('codegen/sam_instructions.py', 'SaM instructions and buffer'), ('codegen/code_generator.py', 'CodeGenerator with generate()')], 'Main Components': [('compiler.py', 'TurboLangCompiler and compilation API'), ('main.py', 'Command-line interface')], 'Documentation': [('README.md', 'Complete documentation')], 'Tests': [('tests/test_lexer.py', 'Lexer tests'), ('tests/test_parser.py', 'Parser tests'), ('tests/test_semantic.py', 'Semantic tests'), ('tests/test_codegen.py', 'Code generation tests')], 'Examples': [('examples/hello.turbo', 'Hello world program'), ('examples/fibonacci.turbo', 'Fibonacci'), ('examples/factorial.turbo', 'Factorial'), ('examples/array_sum.turbo', 'Array operations'), ('examples/control_flow.turbo', 'Control flow')]}
        total = 0
        found = 0
